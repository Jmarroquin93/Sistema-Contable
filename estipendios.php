...
<label id="lblmsg"></label>
<tr><td>ESTIPENDIO</td><td>PARROQUIA</td><td>MINISTRO</td><td>EDITAR</td></tr></thead><tbody>
<?php
...
$sql="call getEstipendios('".$usuario."');";
...
echo"<tr><td><b>$row[2]</td><td><b>$row[3]</td><td><b>$row[4]</td><td><b><button type='button' onclick='GetEstipendios($row[0],\"$row[2]\",$row[3],$row[4])' class='btn btn-primary'>EDITAR</button></td></tr>";
} 
echo"</tbody></table>";
}
...
<script>
 function GetEstipendios(id,estipendio,parroquia,ministro) {
  

    $.ajax({
    type: "POST",
     url: "server.php?p=GetEstipendio",
     data:"id="+id+"&estipendio="+estipendio+"&parroquia="+parroquia+"&ministro="+ministro,
...
<script>
 function updateEstipendio() {
var id= $('#txtId').val();
var estipendio= $('#txtEstipendio').val();
var parroquia= $('#txtParroquia').val();
var ministro= $('#txtMinistro').val();

    $.ajax({
    type: "POST",
     url: "server.php?p=updateEstipendio",
     data:"id="+id+"&estipendio="+estipendio+"&parroquia="+parroquia+"&ministro="+ministro,
     ...
     success: function(response)
       {
          ...
          if(response==0){
            alertify.success('Ok');
    alertify.alert('Exito', 'Estipendio actualizado').set('onok', function(closeEvent){javascript:document.location.reload();} );

        }else{
          alertify.alert('Error', 'Error al actualizar').set('onok', function(closeEvent){javascript:document.location.reload();} ); ;

...
<div class='card-header'>Agregar Cuenta de ".$nombre."</div>
<div class='card-body'>
<button type='button' class='btn btn-success' onclick='add_2_cuentasModal(".$corr.",\"$nombre\",\"$tipo\")'>AGREGAR</button>
</div>
</div>
</div>
</div>
...
<div class="col-12 col-md-12">
<div class="card card-info">
<div class="card-header">Subcuentas Cuentas de '.$nombre.'</div>
<div class="card-body">
<label id="lblmsg"></label>
<table class="table" id="mitablaiglesia"><thead>
<tr><td><b>CORRELATIVO</td><td><b>TIPO CUENTA</td><td><b>NOMBRE</td><td><b>SUB CUENTAS</td><td><b>RETROACTIVA</td></tr></thead><tbody>';
...
$sql="call Get_Cuentas_8_Digitos('".$usuario."','".$correlativo."');";
...
echo"<tr><td><b>$row[0]</td><td><b>$row[2]</td><td><b>$row[1]</td><td><b><a href=agregarCuentas10.php?usuario=".$usuario."&correlativo=$row[0]&tipo=$row[2]&nombre=".urlencode($row[1])." class='btn btn-warning'>SUB CUENTAS</a></td><td><button type='button' class='btn btn-danger' onclick='retroactiva(\"$row[1]\")'>ACTUALIZAR</button></td></tr>";
...
<script>
function retroactiva(c) {

  var cuenta = c;
   $.ajax({
   type: "POST",
     url: "server.php?p=updateRetroactiva",
     data:"cuenta="+cuenta,
     ...
     success: function(response)
        {
          if(response==0)
{
      alertify.alert('Exito', 'Cuenta Actuzalizada con exito!!').set('onok', function(closeEvent){javascript:document.location.reload();} ); ;
        }else{
          alertify.alert('Exito', 'Error al Actulizar!!').set('onok', function(closeEvent){javascript:document.location.reload();} ); ;
     ...
     success: function(response)
        {
          ...
          if(response==0){
            alertify.success('Ok');
    alertify.alert('Exito', 'Cuenta agregada con exito!!').set('onok', function(closeEvent){javascript:document.location.reload();} );

        }else{
          alertify.alert('Error', 'Cuenta Existente!!').set('onok', function(closeEvent){javascript:document.location.reload();} ); ;
